%% demo NLB (NonLinear Bloch-waves) 
% GP equation with periodic potential and periodic BC
%% preparations
close all; clear all;p=[];format compact;
p=stanparam(p); screenlayout(p);
% here set a number of problem parameters in main script, then call init
...
p.sw.bcper=3; % periodicity:   1: top=bottom, 2: left=right; 3: torus
icsw=0; % initial guess switch: 0 for u=0, 1 for asymptotics via eigenfun
...
% here calling swibram_pub to switch of user interaction
pffac=1e3; del=1e-12; 
q=swibram_pub('p','bpt1','q1',0.02,del,nr,pffac);
q.nc.nsteps=10; q.sw.sfem=1; q=cont(q);
q=swibram_pub('p','bpt2','q2',0.02,1e-12,nr,pffac);
q.nc.nsteps=10; q=cont(q);
q=swibram_pub('p','bpt3','q3',0.02,1e-12,nr,pffac);
q.nc.nsteps=10; q=cont(q);
%% plot BD
