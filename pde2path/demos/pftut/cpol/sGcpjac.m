...
u1=u(1:p.nus); u2=u(p.nus+1:p.nus+np2-1); % the 2 fields 
...
u1N=sum(u1(p.i1))/p.nN; u1S=sum(u1(p.i2))/p.nS; 
u1e=[u1; u1N; u1S]; %zeros(p.nub-p.nus-2,1)]; 
f1w=k0+ga*u1e.^2./(1+u1e.^2); 
bsb(1:p.nus,1:p.nus)=-ei*R^2*p.mat.M1*spdiags(f1w(1:p.nus),0,np1,np1);  % bulk-surface-block 
bbb=ei*p.mat.Kb; %+p.mat.Q2(1:end,1:end).*f1w;   % bulk-bulk-block 
bbb(1:p.nus+2,1:p.nus+2)=bbb(1:p.nus+2,1:p.nus+2)...
    +spdiags(p.mat.G2(1:p.nus+2).*f1w,0,p.nus+2,p.nus+2); 

sbb=sparse(p.nu-p.nus,p.nus); % surface-bulk 
sbb(1:p.nus,1:p.nus)=spdiags(p.mat.G2(1:p.nus).*f1u,0,p.nus,p.nus); 
rN=zeros(1,p.nus); rS=rN; rN(p.i1)=1; rS(p.i2)=1; 
