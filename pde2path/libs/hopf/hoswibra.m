...
muv=muv(idx); V=V(:,idx); muv; 
phi=V(:,evnr); phi=phi/norm(phi,2); 
...
p.hopf.y=p.hopf.y+ig; %p.fuha.ufu=@hostanufu; p.fuha.headfu=@hostanheadfu; 
hoplot(p,p.plot.pfig,p.plot.pcmp,p.hopf.aux); figure(p.plot.pfig); title('initial guess');
p.sw.para=para; p.sw.spcalc=0; p=hoMini(p); p.sol.ds=ds; par=p.u(p.nu+1:end); 
if para==3;  p.sol.restart=1;   % ---------- nat. param, use TOM 
  p.hopf.y=[p.hopf.y; 
  ...
  %p.hopf.tom.Stats='off'; p.hopf.tom.Stats_step='off';
  %p.hopf.tom.Stats='on'; p.hopf.tom.Stats_step='on';
else p.sol.restart=0;           % --------- arcl, use honloopext 
...
%  [pc,pc_y]=hopc(p,p.hopf.y,p.hopf.T,lamg); 
  if pcheck % check predictor 
    ...
    f=tomassempbc(p,p.hopf.y,p.hopf.T,p.hopf.lam); 
    normf=norm(f,inf); normy=max(max(abs(y(1:nu,:)-y0(1:nu,:)))); 
    fprintf('max|y-u0|=%g, res=%g\n',normy,normf); 
    p.hopf.y0d=sety0dot(p,p.hopf.y,par,p.hopf.T);
  %  [pc,pc_y]=hopc(p,p.hopf.y,p.hopf.T,p.hopf.lam); 
%    if p.hopf.nqh>0; qf=p.hopf.qfh(p,y); fprintf('q=%g\n',qf); end 
  end
