...
% isc: compute canonical paths to a CSS or CPS, continuation in alpha or arclength
% 
% wrapper for cpsolver. 
% Computes a canonical path from p.u0 to a CPS/CSS p.oc.s1 with sattle
% point property by continuation in the initial state either via natural
% continuation or (pseudo) arclength continuation.
% p: Problem strucure initialized by ocinit, see this for details
...
% solution, FS is the CSS/CPS and al is the current alpha value. Thus alvin
% should start small and end in 1, e.g. alvin=0:0.1:1
% nsteps: (optional) number of arclength steps after the last value in alvin
% is reached. Continuation is automatically aborted if alpha is close to
% one and a last continuation in natural parametrization is made to reach 1
% exactly.
