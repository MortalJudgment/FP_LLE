...
%
bif=0; deta0=p.sol.deta; ds=p.sol.ds; xi=0.5; sds=sign(ds); Gubb=Gu; % use balanced xi for det(A) 
...
switch p.sw.bifcheck
    ...
    case 2; bif=0;  j0l=[]; 
        ...
        try;  if (ineg1(j)~=ineg0(j) && abs(real(muv(j)))<p.nc.mu1); 
                 bif=1; j0l=[j0l, j]; end;  %ineg0, ineg1  
         catch; end 
        end
...
ubb=u1;  % ubb (behind bif) used for next cont if sw.cdbb=1
   ...
   while (bisecc<p.nc.bisecmax && abs(ds)>p.nc.dsminbis) 
     ...
     if(res<2*p.nc.tol) % If step was (rather) OK, then 
      ...
      if p.sw.bifcheck==1; 
         ...
         catch; [L,U]=lu(amat); j0l=1; detab=full(prod(sign(diag(U)))); end; 
        ...
        if ineg(j0)==ineg0(j0); ds=sds*abs(ds)/2; % u_n is before bif, sds~sign(ds0)          
            if wb==1; u1=u0; end; wb=0; % set u1 to u0 if step before was behind bif (wb)          
      ...
      tau0=tau1; u0=un; % set tau, u0 for the next step in bisec 
