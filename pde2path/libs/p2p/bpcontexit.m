...
if ischar(varargin{1}); dir=varargin{1}; fname=varargin{2}; 
   if nargin==3; nname=varargin{3}; nnsw=1; end 
...
p.sol.ds=-1e-2;% p.plot.bpcmp=0; 
p.sw.spcalc=1; p.nc.tol=1e-8; p.sw.bifcheck=1; 
p=cont(p,2); p.sol.ds=-p.sol.ds; p=cont(p,2); % cont in both directions to have tangent for swibra