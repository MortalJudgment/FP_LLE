...
%if p.nc.nq>0; fprintf('BP-cont with nq>0 not yet implemented; add constraints to standard G, see demo CHb.\n'); 
%   a(1)=[0 0]; end % throwing an error! 
...
% Create new initial vector: format is u=[upde; psi; uauxold; auxlin; mu]
...
% inewpar, p.nc.ilam, p.naux+1
p.nc.ilam=[inewpar p.nc.ilam p.naux+1];
%           new        old        mu                         
% set new equation numbers
p.nc.neq=2*p.nc.neq; p.nu=2*p.nu; p.nc.nq=2*p.nc.nq+2; 
p.sw.spcont=p.sol.ptype; p.sol.restart=1;
