...
%
%   [p,t1,ts,nc]=tints(p,dt,nt,pmod,nffu,varargin)
%
% * t0=initial time, ts=time-series (t and residual), dt=step size, 
%   nt= number of steps, plot each pmod-th step, nonlinearity f taken from nffu(p,p.u) 
% * varargin: replacement stiffness matrix.
% * Returns: t1=end time t1, ts=time series, nc=counter
%
